...
namespace Akka.Contrib.Pattern
{
    ...
    internal sealed class OldestChangedBuffer : UntypedActor
    {
        ...
        {
            _membersByAge = state.Members.Where(m => m.Status == MemberStatus.Up && MatchingRole(m)).ToImmutableSortedSet(MemberAgeOrdering.Descending);
            var initial = new InitialOldestState()
                          {
                              Oldest = MemberAddressOrDefault(_membersByAge.FirstOrDefault()),
                              MemberCount = _membersByAge.Count
                          };
    ...
    internal sealed class ClusterSingletonManagerActor : FSM<ClusterSingletonState, IClusterSingletonData>
    {
        ...
        private State<ClusterSingletonState, IClusterSingletonData> HandOverDone(ActorRef handOverTo)
        {
            ...
            if (_selfExited || _removed.ContainsKey(_cluster.SelfAddress))
            {
        ...
        private void InitializeFSM()
        {
                    ...
                    @event.FsmEvent.Match().With<StartOldestChangedBuffer>(
                        buffer =>
                        {
                            ...
                            initialOldestState =>
                            {
                                ...
                                if (_cluster.SelfAddress.Equals(initialOldestState.Oldest))
                                    if (initialOldestState.MemberCount == 1) nextState = GoToOldest();
                                    else
                                        nextState =
            ...
            When(
                ClusterSingletonState.BecomingOldest,
                @event =>
                {
                    State<ClusterSingletonState, IClusterSingletonData> nextState = null;
